...
public static class SeedUsers
{
    ...
    public static async Task Initialize(IServiceProvider serviceProvider, UserInfoData[] seedData, string testUserPw )
    {
        try
        {
            ...
            using (var context = new WatchPartyDbContext(serviceProvider.GetRequiredService<DbContextOptions<WatchPartyDbContext>>()))
            {
                ...
                foreach(var u in seedData)
                {
                    ...
                    var identityID = await EnsureUser(userManager, testUserPw, u.UserName, u.Email, u.EmailConfirmed);
                    // Create a new WatchPartyUser if this one doesn't already exist
                    Watcher w = new Watcher { AspNetIdentityId = identityID, FirstName = u.FirstName, LastName = u.LastName, Email = u.Email, Username = u.UserName };
                    if(!context.Watchers.Any(x => x.AspNetIdentityId == w.AspNetIdentityId && x.FirstName == w.FirstName && x.LastName == w.LastName))
                    {
    ...
    public static async Task InitializeAdmin(IServiceProvider serviceProvider, string email, string userName, string adminPw, string firstName, string lastName)
    {
        try
        {
            using (var context = new WatchPartyDbContext(serviceProvider.GetRequiredService<DbContextOptions<WatchPartyDbContext>>()))
            {
                ...
                var identityID = await EnsureUser(userManager, adminPw, userName, email, true);
                // Create a new WatchPartyUser if this one doesn't already exist
                Watcher w = new Watcher { AspNetIdentityId = identityID, FirstName = firstName, LastName = lastName, Email = email, Username = userName};
                if (!context.Watchers.Any(x => x.AspNetIdentityId == w.AspNetIdentityId && x.FirstName == w.FirstName && x.LastName == w.LastName))
                {
                    // Doesn't already exist, so add a new user
                    context.Add(w);
